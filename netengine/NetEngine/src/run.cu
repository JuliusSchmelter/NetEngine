...
//--------------------------------------------------------------------------------------------------
void NetEngine::Net::run_cuda(float* sample, float** results) {
    // Run first layer.
    size_t threads_per_block = BLOCK_SIZE * BLOCK_SIZE;
    size_t n_blocks = (m_weights[0].rows + threads_per_block - 1) / threads_per_block;

    feed_forward<<<n_blocks, threads_per_block>>>(sample, m_weights[0].data, results[0],
    ...
    for (size_t i = 1; i < m_weights.size(); i++) {
        n_blocks = (m_weights[i].cols + threads_per_block - 1) / threads_per_block;

        feed_forward<<<n_blocks, threads_per_block>>>(results[i - 1], m_weights[i].data, results[i],
                                                      m_weights[i].cols - 1, m_weights[i].rows);
...
std::vector<float> NetEngine::Net::run(const std::vector<float>& sample) {
    ...
    for (size_t i = 0; i < m_weights.size(); i++) {
        TRY_CUDA(cudaMalloc(&results_cuda[i], m_weights[i].cols * sizeof(float)));
    }
    ...
    TRY_CUDA(cudaMemcpy(output.data(), results_cuda[m_weights.size() - 1],
